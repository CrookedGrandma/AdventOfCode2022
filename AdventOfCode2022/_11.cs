...
public class Monkey {
    ...
    public void ThrowItems(List<Monkey> monkeys, int round) {
        while (items.Count > 0) {
            ThrowFirstItem2(monkeys, round);
        }
    }
    ...
    private void ThrowFirstItem2(List<Monkey> monkeys, int round) {
        ulong item = items.Dequeue();
        ulong worry = operation(item) % 9699690;
        this.inspections++;
        if (worry % divisibility == 0)
            monkeys[monkeTrue].items.Enqueue(worry);
        else
